...
export async function uploadPhoto(file: File, purpose: "task" | "chat", applicationId?: string): Promise<Photo> {
  if (file.size > 3 * 1024 * 1024) throw new Error("Фото должно быть не больше 3 МБ.");
  const form = new FormData(); form.set("photo", file); form.set("purpose", purpose);
  if (applicationId) form.set("applicationId", applicationId);
  const response = await fetch("/api/media", { method: "POST", body: form });
  const result = await response.json() as Photo & { error?: string }; if (!response.ok) throw new Error(result.error || "Не удалось загрузить фото.");
  return result;
}
export function TaskCreate({ onCreated, onBusy }: { onCreated: () => void; onBusy: (busy: boolean) => void }) {
  ...
  return <div className="modal-content"><div className="modal-eyebrow">НОВОЕ ОБЪЯВЛЕНИЕ</div><h2>Что нужно сделать?</h2><form className="modal-form" onSubmit={submit}>
    <label>Заголовок<input name="title" required minLength={4} maxLength={120} placeholder="Например, смонтировать видео" /></label>
    <label>Описание<textarea name="description" required minLength={10} maxLength={5000} placeholder="Что нужно сделать и к какому сроку?" /></label>
    <div className="photo-upload"><strong>Фотографии <small>до 5 · JPEG, PNG, WebP · до 3 МБ</small></strong><div className="photo-previews">{photos.map((photo, index) => <div key={photo.id}><img src={photo.url} alt={`Фотография ${index + 1}`} /><button type="button" disabled={uploading || saving} aria-label={`Убрать фотографию ${index + 1}`} onClick={() => setPhotos((current) => current.filter((p) => p.id !== photo.id))}><X size={15} /></button></div>)}{photos.length < 5 && <label className="photo-add"><ImagePlus size={24} /><span>{uploading ? "Загрузка…" : "Добавить"}</span><input type="file" accept="image/jpeg,image/png,image/webp" multiple disabled={uploading || saving} onChange={(e) => { void addPhotos(e.target.files); e.target.value = ""; }} /></label>}</div></div>
    <div className="form-grid"><label>Категория<select name="category" defaultValue="digital"><option value="digital">Диджитал</option><option value="delivery">Доставка</option><option value="repair">Ремонт</option><option value="home">Дом и быт</option><option value="study">Учёба</option></select></label><label>Бюджет, ₽<input name="price" type="number" min="0" max="10000000" step="1" required placeholder="1500" /></label></div>
    ...
    <label className="urgent-check"><input name="urgent" type="checkbox" /><span><Zap size={17} />Нужно срочно<small>Выше в рекомендациях. Расчётная комиссия — 12% вместо 7%; списания пока не подключены.</small></span></label>
    {error && <p className="field-error" role="alert">{error}</p>}<button className="primary-button wide" disabled={saving || uploading}>{saving || uploading ? <Loader2 size={18} className="spin" /> : <Plus size={18} />}Опубликовать объявление</button>
  </form></div>;
}
